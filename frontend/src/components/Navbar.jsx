...
import { Bell, Search, LayoutDashboard, Target, Home, Settings } from 'lucide-react'
    ...
    <nav className="bg-surface/80 backdrop-blur-md border-b border-secondary-200 sticky top-0 z-50">
      <div className="max-w-6xl mx-auto px-4 h-16 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4 border-l border-slate-200 pl-4">
          ...
          <SignedIn>
            {/* Mock notification icon common in SaaS */}
            <button className="text-slate-400 hover:text-slate-600 transition-colors relative mr-2">
              <Bell className="w-5 h-5" />
              <span className="absolute max-w-2 top-0 right-0 transform translate-x-1/2 -translate-y-1/2 w-2 h-2 bg-rose-500 border border-white rounded-full"></span>
            </button>
